...
const rateLimitedGetHandler = withRateLimit(RATE_LIMIT_CONFIGS.api, (request: NextRequest) => {
  // Extract IP address for rate limiting
  const forwarded = request.headers.get('x-forwarded-for')
  const ip = forwarded ? forwarded.split(',')[0] : 'unknown'
  return `prompts_get_id:${ip}`
})(async (
  request: NextRequest,
...
const rateLimitedPutHandler = withRateLimit(RATE_LIMIT_CONFIGS.api, (request: NextRequest) => {
  // Extract IP address for rate limiting
  const forwarded = request.headers.get('x-forwarded-for')
  const ip = forwarded ? forwarded.split(',')[0] : 'unknown'
  return `prompts_put_id:${ip}`
})(async (
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) => {
...
const rateLimitedDeleteHandler = withRateLimit(RATE_LIMIT_CONFIGS.api, (request: NextRequest) => {
  // Extract IP address for rate limiting
  const forwarded = request.headers.get('x-forwarded-for')
  const ip = forwarded ? forwarded.split(',')[0] : 'unknown'
  return `prompts_delete_id:${ip}`
})(async (
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) => {
